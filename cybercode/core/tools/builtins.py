...
=====================================================================
主要功能：
1. 提供系统级内置工具（时间、计算、记忆、任务等）
2. 封装沙盒工具（文件读写、Shell执行）
3. 定义 tasks_lock 线程锁（用于定时任务文件操作）

工具列表：
- get_system_model_info: 获取当前模型信息
- save_user_profile: 保存用户画像
- read_user_profile: 读取用户画像
- get_current_time: 获取当前时间
- calculator: 计算器
- list_office_files: 列出沙盒文件
- read_office_file: 读取沙盒文件
- write_office_file: 写入沙盒文件
- execute_office_shell: 执行Shell命令
- add_task: 添加定时任务
- list_tasks: 列出定时任务
- delete_task: 删除定时任务
=====================================================================
...
import uuid
import threading
from ..config import MEMORY_DIR, TASKS_FILE, PERSONAS_DIR
...
# =====================================================================
# tasks_lock - 定时任务文件的线程锁
# =====================================================================
# 防止多线程同时读写 tasks.json 导致数据损坏
...
def get_system_model_info() -> str:
    """
    获取当前 cybercode 正在运行的底层大模型（LLM）型号和提供商信息。
...
def load_personas() -> str:
    """
    加载所有可用的 persona 文件。
    返回 personas 目录下所有 .md 文件的名称和内容。
    如果目录下没有 persona 文件，返回提示信息。
    """
    ...
    if not persona_files:
        return "当前没有可用的 persona 文件。请在 workspace/personas/ 目录下创建 .md 文件来定义 persona。"
    ...
    return "已加载的 Personas:\n\n" + "\n\n---\n\n".join(result)


@cyberclaw_tool
def save_my_persona(filename: str, content: str) -> str:
    """
    保存 AI 自己的 persona 文件。
    用于 AI 在获得用户同意后更新自己的角色设定。
    
    参数:
    - filename: persona 文件名（不含扩展名），如 "cybercode_persona"
    - content: persona 的完整 Markdown 内容
    """
    os.makedirs(PERSONAS_DIR, exist_ok=True)
    ...
    return f"Persona 文件 [{filename}] 已成功保存到 workspace/personas/ 目录。新的人设已生效。"
