...
from cybercode.core.context import AgentState
from langchain_core.messages import HumanMessage, AIMessage, ToolMessage


class TestAgent(unittest.TestCase):

    def test_session_visibility_note_contains_thread_and_recent_user_messages(self):
        """测试会话可见性提示包含 thread_id 和最近用户消息"""
        from cybercode.core.agent import _build_session_visibility_note
        ...
        note = _build_session_visibility_note("local_geek_master", messages)

        self.assertIn("local_geek_master", note)
        self.assertIn("当前可见历史消息数: 3", note)
        self.assertIn("第一个问题", note)
        self.assertIn("第二个问题", note)
        self.assertIn("不要声称自己完全看不到会话或历史", note)
    ...
    @patch('cybercode.core.provider.get_provider')
    @patch('cybercode.core.skill_loader.load_dynamic_skills')
    @patch('cybercode.core.tools.builtins.BUILTIN_TOOLS', [])
