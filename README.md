...
[![Based on CyberClaw](https://img.shields.io/badge/Based_on-CyberClaw-111111.svg)](https://github.com/ttguy0707/CyberClaw)

**基于 CyberClaw 二次开发的终端智能体** · A customized terminal-first AI agent built on CyberClaw

[二次开发说明](#-二次开发说明) · [快速开始](#-快速开始) · [核心能力](#-核心能力) · [架构图](#-系统架构)

</div>

***

> 🙏 **致谢原项目**：本项目基于 [CyberClaw](https://github.com/ttguy0707/CyberClaw) 的 MIT 开源代码进行二次开发。\
> 原项目提供了透明审计、记忆系统、心跳任务、技能机制等核心架构；感谢原作者开放源码，让本项目可以在其基础上继续扩展。
>
> 💡 **上游灵感来源**：CyberClaw 受 [OpenClaw](https://github.com/openclaw/openclaw) 启发，聚焦 AI 智能体的透明度与可控性。

***

## 🙌 二次开发说明

`Cyber Code` 是在 `CyberClaw` 基础上继续演进的个人二次开发版本。当前版本重点补充了更完整的终端体验、品牌化能力和用户自定义能力。

### 本项目新增能力

| 功能             | 说明                                                                |
| -------------- | ----------------------------------------------------------------- |
| **预览命令**       | 新增 commands 命令模块，如 `/skills`、`/resume`、`/help` 等                  |
| **本地对话切换**     | 新增/resume命令，实现切换对话                                                |
| **终端主题系统**     | 新增 `acid-pulse`、`neon-grid`、`signal-burn` 三套主题，并支持 `/theme` 持久化切换 |
| **Persona 加载** | 支持从 `workspace/personas/` 读取 AI 自我设定，让助手具备可维护的人设层                 |
| **终端视觉重构**     | 重新设计启动 Banner、配置向导、监控面板和整体色彩体系                                    |
| **技能加载器增强**    | 在既有动态技能机制上补充懒加载、元数据缓存与热更新能力                                       |

### 与原项目关系

- 本项目保留 MIT License
- 原始架构、动态技能机制与大量基础能力来自 `CyberClaw`
- 本仓库中的新增实现、品牌化改造和后续扩展由本项目维护者继续开发

***

## 📖 简介

Cyber Code 继承了 CyberClaw 的透明可控智能体架构，并在其基础上继续扩展终端体验：
...
Cyber Code 继承了上游的动态技能机制，并支持 **OpenClaw 技能**和 **Claude Code 技能**，可直接使用两个生态系统的丰富技能资源，无需重新开发。
...
- **双水位记忆系统**
  ...
  - 近期摘要 (SQLite)：每 MAX\_TURNS 轮自动摘要，保留最近 KEEP\_TURNS 轮
...
- **推荐技能**：
  - `skill-creator`：用自然语言让 Cyber Code 自己创建技能
...
# Base URL (可选，使用代理时配置)
OPENAI_API_BASE=https://coding.dashscope.aliyuncs.com/v1 （阿里云默认 Base URL）
```
...
> 💡 **工作区配置**：工作区路径已在代码中初始化，默认为项目根目录的 `workspace` 文件夹，无需在 `.env` 中配置。仅当需要自定义工作区位置时，才设置对应工作区环境变量。

> 💡 提示：配置完成后，可运行 `cybercode run` 聊天测试连接是否正常。
...
| 模块           | 文件                                      | 功能                        |
| ------------ | --------------------------------------- | ------------------------- |
| **Agent 循环** | `cybercode/core/agent.py`               | LangGraph StateGraph，决策大脑 |
| **技能加载**     | `cybercode/core/skill_loader.py`        | 动态加载 SKILL.md，两段式调用       |
| **上下文管理**    | `cybercode/core/context.py`             | 消息修剪，双水位记忆                |
| **内置工具**     | `cybercode/core/tools/builtins.py`      | 时间/计算/任务调度等               |
| **沙盒工具**     | `cybercode/core/tools/sandbox_tools.py` | 文件操作 + Shell 执行           |
| **审计日志**     | `cybercode/core/logger.py`              | JSONL 格式事件记录              |
| **心跳任务**     | `cybercode/core/heartbeat.py`           | 定时任务检查与触发                 |
...
```bash
# 克隆项目
git clone https://github.com/BIngo-He/cybercode.git
cd cybercode
...
- **[CyberClaw](https://github.com/ttguy0707/CyberClaw)** - 本项目的上游基础，感谢原作者开源核心架构
- **[OpenClaw](https://github.com/openclaw/openclaw)** - 上游灵感来源与技能生态
...
**👾 Cyber Code · 基于 CyberClaw 二次开发**

Built on top of [CyberClaw](https://github.com/ttguy0707/CyberClaw)
